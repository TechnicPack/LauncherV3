...
class DownloadFilePlanActionTest {
  ...
  void prefixesRawFileNamesWithDownloading() {
    assertEquals(
        "Downloading modpack.zip", DownloadFilePlanAction.formatProgressLabel("modpack.zip"));
  }

  @Test
  void preservesExistingActionLabels() {
    assertEquals(
        "Extracting libraries.zip...",
        DownloadFilePlanAction.formatProgressLabel("Extracting libraries.zip..."));
    assertEquals(
        "Download failed, retries remaining: 2",
  ...
  void usesTaskDescriptionVerbatimWhenConfiguredForPlannedLabels() {
    ...
    assertEquals(
        "Launcher Asset: discover.json",
        DownloadFilePlanAction.buildProgressLabel(
            "discover.json", "Launcher Asset: discover.json", true));
    assertEquals(
        "Downloading temp.jar", DownloadFilePlanAction.buildProgressLabel("temp.jar", "   ", true));
  }
}
