...
public class ModpackDataDisplay extends JPanel implements IImageJobListener<ModpackModel> {
  ...
  public ModpackDataDisplay(
      ResourceLoader resources, ImageRepository<ModpackModel> logoRepo, IDiscordApi api) {
  ...
  public void setModpack(ModpackModel modpack) {
    ...
    if (this.packSiteUrl == null) this.packSiteUrl = "https://www.technicpack.net/";
    ...
    if (wasVisible == modpack.isOfficial()) {
      if (wasVisible) statBoxes.remove(ratings);
      else statBoxes.add(ratings, 0);
    }
  ...
  private void updateDiscordComponents(ModpackModel pack, Server server) {
    ...
    // The current modpack check should also run in the EDT because the current modpack might change
    // while
    ...
    // If the pack has changed since we requested the Discord server information, ignore the
    // response
    ...
    this.countLabel.setText(
        resources.getString("launcher.discord.count", Integer.toString(server.getPresenceCount())));
