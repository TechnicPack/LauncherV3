...
public class OptionsDialog extends LauncherDialog implements IRelocalizableResource {
  ...
  private final DocumentListener javaArgsListener =
  ...
  private final DocumentListener dimensionListener =
      new DocumentListener() {
        @Override
        public void insertUpdate(DocumentEvent e) {
  ...
  private final DocumentListener wrapperCommandListener =
      new DocumentListener() {
        @Override
        public void insertUpdate(DocumentEvent e) {
  ...
  protected void selectOtherVersion() {
    ...
    if (result == JFileChooser.APPROVE_OPTION) {
      if (chooser.getSelectedFile() == null
          ...
          || !chooser.getSelectedFile().canExecute()) {
      ...
      if (!javaVersions.addVersion(chosenJava)) {
        JOptionPane.showMessageDialog(
            this, resources.getString("launcheroptions.java.versionexists"));
        return;
      }
  ...
  private void initControlValues() {
    ...
    if (best64Bit != null) versionSelect.addItem(new Best64BitVersionItem(best64Bit, resources));
    ...
    if (settingsVersion == null
        || settingsVersion.isEmpty()
        || settingsVersion.equals(JavaVersionRepository.VERSION_DEFAULT))
      ...
      for (int i = 2; i < versionSelect.getItemCount(); i++) {
        if ((versionSelect.getItemAt(i)).getVersionNumber().equals(settingsVersion)
            && (versionSelect.getItemAt(i)).is64Bit() == settingsBitness) {
    ...
    launchSelect.addItem(
        new OnLaunchItem(
            resources.getString("launcheroptions.packlaunch.hide"), LaunchAction.HIDE));
    launchSelect.addItem(
        new OnLaunchItem(
            resources.getString("launcheroptions.packlaunch.close"), LaunchAction.CLOSE));
    launchSelect.addItem(
        new OnLaunchItem(
            resources.getString("launcheroptions.packlaunch.nothing"), LaunchAction.NOTHING));
    ...
    langSelect.setRenderer(
        new LanguageCellRenderer(
            resources, null, langSelect.getBackground(), langSelect.getForeground()));
    ...
    if (settings.shouldUseStencilBuffer()) useStencil.setSelectedIndex(0);
    else useStencil.setSelectedIndex(1);
      ...
      if (is64Bit) {
        ...
        toolTip.setBorder(
            BorderFactory.createCompoundBorder(
                new LineBorder(UIConstants.COLOR_GREY_TEXT),
  ...
  private void initComponents() {
    ...
    String linkText =
        "<a href=\"https://github.com/TechnicPack/\">"
            + resources.getString("launcheroptions.about.linktext")
            + "</a>";
    String aboutText =
        "<html><head><style type=\"text/css\">a{color:#309aeb}body{font-family: "
            + resources.getFont(ResourceLoader.FONT_OPENSANS, 12).getFamily()
            + ";color:#D0D0D0}</style></head><body>";
    aboutText +=
        "<p>"
            + resources.getString(
    ...
    centerPanel.addTab(
        resources.getString("launcheroptions.tab.video").toUpperCase(), videoOptions);
  ...
  private void setupGeneralPanel(JPanel panel) {
    ...
    RoundedButton reinstallButton =
        new RoundedButton(resources.getString("launcheroptions.install.change"));
    ...
    panel.add(
        ...
        new GridBagConstraints(
            ...
            GridBagConstraints.CENTER,
            GridBagConstraints.NONE,
            new Insets(16, 16, 0, 0),
            0,
            0));

    // Add launch to modpacks
    JLabel launchToModpacksField =
        new JLabel(resources.getString("launcheroptions.general.modpacktab"));
  ...
  private void setupVideoOptionsPanel(JPanel panel) {
    ...
    stencilInfo.setText(
        "<html><body style=\"font-family:"
            + stencilInfo.getFont().getFamily()
            + ";color:#D0D0D0\">"
            + resources.getString("launcheroptions.video.stencil.info")
            + "</body></html>");
  ...
  private void setupJavaOptionsPanel(JPanel panel) {
    ...
    RoundedButton otherVersionButton =
        new RoundedButton(resources.getString("launcheroptions.java.otherversion"));
    ...
    JLabel useMojangJavaLabel =
        new JLabel(resources.getString("launcheroptions.java.useMojangJava"));
