...
public class FixRunDataDialog extends LauncherDialog {
  ...
  public FixRunDataDialog(
      Frame owner,
      ResourceLoader resourceLoader,
      RunData runData,
      JavaVersionRepository javaVersionRepository,
      Memory attemptedMemory,
      boolean shouldAskFirst,
  ...
  public void setVisible(boolean visible) {
    if ((recommendedVersion != null
            ...
            && javaVersionRepository.getSelectedVersion().equals(recommendedVersion)
            && attemptedMemory.getMemoryMB() == recommendedMemory.getMemoryMB())
  ...
  public boolean shouldRemember() {
    if (rememberThis == null) return false;
  ...
  private void initComponents() {
    ...
    JLabel label =
        new JLabel(
            "<html>"
                ...
                + font.getFamily()
                + ";font-size:11px;font-color:#D0D0D0\">"
    ...
    boolean isFailure =
        recommendedMemory == null || (!usingMojangJava && recommendedVersion == null);
    ...
    centerPanel.add(
        ...
        new GridBagConstraints(
            ...
            new Insets(isFailure ? 14 : 0, 0, 0, 0),
            0,
            0));
  ...
  private void buildSuccessFailPanels(JPanel centerPanel, String fontFamily) {
    ...
    boolean javaSuccess =
        usingMojangJava
            || runData.isJavaValid(javaVersionRepository.getSelectedVersion().getVersion());

    String memRequirement =
        resourceLoader.getString("fixRunData.reqMemory", runData.getMemoryObject().toString());
    String javaRequirement =
        resourceLoader.getString(
            "fixRunData.reqJava", runData.getJava() == null ? "" : runData.getJava());

    String currentMem =
        resourceLoader.getString("fixRunData.currentMemory", attemptedMemory.toString());
    String currentJavaBitness =
        javaVersionRepository.getSelectedVersion().is64Bit()
            ? resourceLoader.getString("launcheroptions.java.64bit")
            : resourceLoader.getString("launcheroptions.java.32bit");
    String currentJava =
        resourceLoader.getString(
            "fixRunData.currentJava",
            javaVersionRepository.getSelectedVersion().getVersion(),
            currentJavaBitness);
    ...
    if (!javaSuccess && recommendedVersion != null) {
      ...
      String javaBitness =
          recommendedVersion.is64Bit()
              ? resourceLoader.getString("launcheroptions.java.64bit")
              : resourceLoader.getString("launcheroptions.java.32bit");
      if (javaVersionRepository.getBest64BitVersion() == recommendedVersion)
        javaVersion =
            resourceLoader.getString(
                "launcheroptions.java.best64version", javaVersion + " " + javaBitness);
  ...
  private int buildFailureReasons(JPanel centerPanel, int gridBagIndex, String fontFamily) {
    ...
    if (getBetterJava) {
      label =
          new JLabel(
              "<html>"
                  + "<head><style type=\"text/css\">a{color:#309aeb}body{font-family: "
                  + fontFamily
                  + ";font-size:11px;color:#D0D0D0}</style></head><body>"
                  + resourceLoader.getString("fixRunData.needBetterJava")
                  + "</body>"
                  + "</html>",
              resourceLoader.getIcon("danger_icon.png"),
              SwingConstants.LEFT);
  ...
  private void addSuccessFailPanel(
      JPanel centerPanel,
      String fontFamily,
      int gridBagRow,
      boolean isSuccess,
    ...
    JLabel checkbox =
        new JLabel(resourceLoader.getIcon(getPanelIcon(isSuccess, hasRecommendation)));
    successFailPanel.add(
        checkbox,
        new GridBagConstraints(
            0,
            0,
    ...
    JLabel label =
        new JLabel(
            "<html>"
                + "<body style=\"font-family:"
                + fontFamily
                + ";font-size:12px;font-color:#D0D0D0\">"
                + compareText
                + "</body>"
                + "</html>");
    label.setForeground(UIConstants.COLOR_WHITE_TEXT);
  ...
  private Color getPanelColor(boolean isSuccess, boolean hasRecommended) {
    return isSuccess
        ? UIConstants.COLOR_REQUIREMENT_SUCCEED
        : (hasRecommended
            ? UIConstants.COLOR_REQUIREMENT_WARNING
            : UIConstants.COLOR_REQUIREMENT_FAIL);
  }
