...
public class DiscoverInfoPanel extends JPanel {
  ...
  public DiscoverInfoPanel(
      final ResourceLoader loader,
      String discoverUrl,
      final IPlatformApi platform,
      final LauncherFileSystem fileSystem,
      final ModpackSelector modpackSelector) {
    this.fileSystem = fileSystem;
    this.resources = loader;

    if (discoverUrl == null) discoverUrl = "https://api.technicpack.net/discover/";
    ...
    SwingUtilities.invokeLater(
        () -> {
          try {
            ...
            panel.setDocument(
                getDiscoverDocument(runnableAccessDiscover, localCache), runnableAccessDiscover);
          } catch (Exception e) {
  ...
  protected Document getDiscoverDocument(String url, Path localCache) {
    // Attempt to retrieve the discover page from the live site, then a local cache, then an
    // internal resource
