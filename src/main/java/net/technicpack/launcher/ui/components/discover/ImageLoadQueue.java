...
/**
 * A thread-safe queue containing BackgroundImageLoaderItem, each of which represents one image
 * (identified by a URI) which needs to be loaded.
 */
class ImageLoadQueue {
   ...
   *
   * @param uri URI of the item to be loaded. As there is no good way of reporting failures, you
   *     should ensure the URI is a proper URL before calling this method.
   * @param re container for the image to be loaded; will be updated via {@link
   *     MutableFSImage#setImage(String,java.awt.Image,boolean)} once image is loaded public
   *     synchronized void addToQueue(String uri, MutableFSImage re) { this.addToQueue(uri, re, -1,
   *     -1); }
   */
  ...
  /**
   * Returns the next available task from the queue, or blocks if there are no more; items are
   * returned in FIFO order. If none are available, the method will block until the next items is
  ...
  /**
   * Indicates that no more items will be added to the queue, no more items currently in the queue
   * will be loaded, and that worker threads polling this queue should shut down.
   */
  ...
  /**
   * Returns true if the item, retrieved from the queue via {@link #getTask()}, is a kill switch,
   * meaning the worker that retrieved it should stop polling.
   *
   * @param queueItem an item retrieved from the queue.
   * @return true if the item, retrieved from the queue via {@link #getTask()}, is a kill switch,
   *     meaning the worker that retrieved it should stop polling.
   */
