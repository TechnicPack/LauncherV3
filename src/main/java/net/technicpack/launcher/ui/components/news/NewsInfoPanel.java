...
public class NewsInfoPanel extends JPanel implements PropertyChangeListener {
  ...
  public void setArticle(NewsArticle article) {
    ...
    newsText.setText(
        "<html><head><style type=\"text/css\">a{color:#309aeb}body{font-family:\""
            + newsText.getFont().getFamily()
            + "\";color:#D0D0D0}</style></head><body>"
            + article.getContent()
            + "</body></html>");

    authorshipInfo.setAuthorshipInfo(
        article.getAuthorshipInfo(), avatarRepo.startImageJob(article.getAuthorshipInfo()));
  ...
  private void initComponents() {
    ...
    newsText.addHyperlinkListener(
        e -> {
          if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
            if (e.getURL() != null) DesktopUtils.browseUrl(e.getURL().toString());
          }
        });
