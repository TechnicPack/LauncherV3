...
public class LoginFrame extends DraggableFrame implements IRelocalizableResource, IAuthListener {
  ...
  public void userChanged(IUserType user) {
    if (user == null) {
      ...
      if (nameSelect.isVisible()) nameSelect.grabFocus();
  ...
  private void initComponents() {
    ...
    //        add(visitBrowser,  new GridBagConstraints(0, 2, 3, 1, 0.0, 0.0,
    ...
    UserCellEditor userEditor =
        new UserCellEditor(
            resources.getFont(ResourceLoader.FONT_OPENSANS, 16),
            this.skinRepository,
            UIConstants.COLOR_BUTTON_BLUE);
  ...
  protected void refreshSelectedUsers() {
    ...
    if (users.isEmpty()) {
      ...
      if (lastUser == null) lastUser = users.iterator().next();
  ...
  private void login(IUserType user) {
    ...
    try {
      ...
      showMessageDialog(
          this,
          "Please log in again for user " + user.getDisplayName(),
          e.getMessage(),
          ERROR_MESSAGE);
      forgetUser(user);
      ...
      try {
        ...
        switch (e.getType()) {
          ...
          case DNS:
            showMessageDialog(
                parent,
                "DNS failure: " + e.getMessage(),
                "DNS failure during authentication",
                ERROR_MESSAGE);
            break;
          default:
            showMessageDialog(
                parent, e.getMessage(), "Add Microsoft Account Failed", ERROR_MESSAGE);
        }
      }
