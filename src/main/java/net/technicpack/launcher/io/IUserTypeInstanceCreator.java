...
public class IUserTypeInstanceCreator
    implements JsonDeserializer<IUserType>, JsonSerializer<IUserType> {
  ...
  @Override
  public IUserType deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    String userString =
        rootObject.has(USER_TYPE_FIELD)
            ? rootObject.get(USER_TYPE_FIELD).getAsString().toLowerCase(Locale.ROOT)
            : null;
