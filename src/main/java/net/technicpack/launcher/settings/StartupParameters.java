...
public final class StartupParameters {
  ...
  @Parameter(
      names = "-launcheronly",
      description = "Start in launcher mode (rather than update/mover)")
  private boolean launcher = false;

  @Parameter(
      names = "-launcher",
      description = "Legacy launcher mode (indicates we should do a full update)")
  private boolean oldLauncher = false;

  @Parameter(
      names = "-moveronly",
      description =
          "Start in mover mode (copies recently-downloaded update to originally-run package)")
  private boolean mover = false;

  @Parameter(
      names = "-mover",
      description = "Legacy mover mode (used to detect old-updater clients trying to update)")
  private boolean oldMover = false;

  @Parameter(
      names = "-update",
      description = "Start in update mode (closes after downloading updated resources)")
  private boolean update = false;

  @Parameter(
      names = "-movetarget",
      description = "The path of the originally-run package to copy to")
  ...
  @Parameter(
      names = "-blockReboot",
      description = "Prevent rebooting the launcher due to bad java properties")
  ...
  @Parameter(
      names = "-skipUpdate",
      description = "Skip the launcher update (still updates resources)")
  ...
  public boolean isLegacyLauncher() {
    return oldLauncher;
  }
