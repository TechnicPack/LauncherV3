...
public class UpdatePlanner {
    ...
    public ExecutionPlan<UpdateContext> planUpdater(IUpdateStream updateStream, UpdatePlanRequest request) throws IOException {
        ...
        for (LauncherResource resource : version.getResources()) {
            ...
            builder.addNode(getResourceNodeId(resource), DOWNLOADS_PHASE, resourceDescription, 1.0f,
        ...
        String updateUrl = request.getRunningPath().endsWith(".exe") ? version.getExeUrl() : version.getJarUrl();

        builder.addNode("download-launcher-update", DOWNLOADS_PHASE, request.getLauncherUpdateDescription(), 1.0f,
                (context, reporter) -> {
                    DownloadFilePlanAction<UpdateContext> action =
                            new DownloadFilePlanAction<UpdateContext>(updateUrl, request.getTempLauncher().toFile(), null,
        ...
        if (!request.isUpdateOnly()) {
            ...
            builder.addNode("launch-mover", ACTIONS_PHASE, request.getLaunchMoverDescription(), 0.0f,
                    java.util.Collections.singletonList("download-launcher-update"),
                    (context, reporter) -> context.getRelauncher().launch(request.getTempLauncher().toAbsolutePath().toString(),
    ...
    public ExecutionPlan<UpdateContext> planMover(MoverPlanRequest request) {
        ...
        builder.addNode("copy-launcher-package", ACTIONS_PHASE, request.getMoveDescription(), 1.0f,
                (context, reporter) -> context.getRelauncher().copyToMoveTarget(request.getMoveTarget()));
        builder.addNode("launch-launcher-mode", ACTIONS_PHASE, request.getFinalLaunchDescription(), 0.0f,
                java.util.Collections.singletonList("copy-launcher-package"),
                (context, reporter) -> context.getRelauncher().launch(request.getMoveTarget().toAbsolutePath().toString(),
    ...
    private static DownloadFilePlanAction<UpdateContext> createResourceDownloadAction(LauncherResource resource,
                                                                                      File targetFile,
                                                                                      IFileVerifier verifier,
                                                                                      String taskDescription) throws DownloadException {
        String zstdUrl = resource.getZstdUrl();
        if (zstdUrl != null && !zstdUrl.isEmpty()) {
            return new DownloadFilePlanAction<UpdateContext>(zstdUrl, targetFile, verifier, taskDescription)
                    .withDecompressor(CompressorStreamFactory.ZSTANDARD)
                    .withTaskDescriptionAsProgressLabel();
        }

        return new DownloadFilePlanAction<UpdateContext>(resource.getUrl(), targetFile, verifier, taskDescription)
                .withTaskDescriptionAsProgressLabel();
    }
    ...
    public static class UpdatePlanRequest {
        ...
        public UpdatePlanRequest(String streamName, int currentBuild, boolean skipUpdate, boolean updateOnly,
                                 String runningPath, Path tempLauncher, String downloadsPhaseLabel,
                                 String launchMoverDescription, String launcherAssetDescriptionTemplate,
        ...
        public String getLauncherAssetDescription(String filename) {
            if (launcherAssetDescriptionTemplate == null || launcherAssetDescriptionTemplate.trim().isEmpty()) {
    ...
    public static class MoverPlanRequest {
        ...
        public MoverPlanRequest(Path moveTarget, boolean legacyMover, String moveDescription, String finalLaunchDescription) {
