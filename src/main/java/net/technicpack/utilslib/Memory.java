...
public class Memory {
  ...
  public static final Memory DEFAULT_MEM = memoryOptions[8];
  public static final int MAX_32_BIT_MEMORY = 1024;
  private static final Map<Integer, Memory> MEMORY_BY_SETTINGS_ID = buildMemoryBySettingsId();
  ...
  public static long getPhysicalMemory() {
    ...
    try {
      ...
      if (osInfo instanceof com.sun.management.OperatingSystemMXBean) {
        maxMemory =
            ((com.sun.management.OperatingSystemMXBean) osInfo).getTotalPhysicalMemorySize()
                / 1024
                / 1024;
      } else {
        Utils.getLogger()
            .log(
                Level.SEVERE,
                "Failed to detect physical memory: incorrect instance type "
                    + osInfo.getClass().getName());
      }
    } catch (Throwable t) {
  ...
  public static Memory getClosestAvailableMemory(Memory memory, long available) {
  ...
  public static Memory getMemoryFromId(int id) {
    return MEMORY_BY_SETTINGS_ID.getOrDefault(id, DEFAULT_MEM);
  }
  ...
  private static Map<Integer, Memory> buildMemoryBySettingsId() {
    Map<Integer, Memory> memoryBySettingsId = new HashMap<>();
    for (Memory memory : memoryOptions) {
      memoryBySettingsId.put(memory.getSettingsId(), memory);
    }
    return Collections.unmodifiableMap(memoryBySettingsId);
  }
}
