...
public class ZipUtils {
  public static boolean extractFile(File zip, File output, String fileName)
    ...
    try (ZipFile zipFile = ZipFile.builder().setFile(zip).get()) {
      ...
      if (entry == null) {
        Utils.getLogger()
            .log(Level.WARNING, "File " + fileName + " not found in " + zip.getAbsolutePath());
        return false;
      }
      ...
      if (!outputFile.toPath().normalize().startsWith(output.toPath())) {
        Utils.getLogger()
            .log(
                Level.SEVERE,
                "Detected Zip Slip attempt on entry "
                    + entry.getName()
                    + " from "
                    + zip.getAbsolutePath());
        return false;
  ...
  private static void unzipEntry(ZipFile zipFile, ZipArchiveEntry entry, File outputFile)
  ...
  public static void unzipFile(
    ...
    try (ZipFile zipFile = ZipFile.builder().setFile(zip).get()) {
      ...
      // Commons Compress doesn't seem to throw exception when ZIP files aren't valid, so we just
      // check if they
      // have no entries as a means of validating it, and throw a ZipException so the launcher will
      // show the correct
      // warning message when trying to install/update the modpack, rather than just printing it to
      // console.
      if (totalEntries == 0) {
      ...
      for (ZipArchiveEntry entry : entries) {
          ...
          if (outputFile.exists() && outputFile.isFile() && !outputFile.canWrite()) {
            if (outputFile.delete()) {
              ...
              throw new IOException(
                  "Failed to delete non-writable file " + outputFile.getAbsolutePath());
            }
          }
        ...
        if (listener != null) {
          float totalProgress = (float) entriesDone / totalEntries;
