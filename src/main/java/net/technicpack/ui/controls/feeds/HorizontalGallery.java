...
public class HorizontalGallery extends JPanel {
    ...
    if (!hasNoComponents) {
      ...
      if (getComponentCount() != 0) {
        ...
        if (lastDisplayedComponent != getComponents()[getComponentCount() - 1]) {
          g2d.setPaint(
              new GradientPaint(
                  getWidth() - 80, 0, new Color(0, 0, 0, 0), getWidth() - 10, 0, flatBackground));
          g2d.fillRect(0, 0, getWidth(), getHeight());
        }
      }
  ...
  public Component getSelectedComponent() {
    ...
    if (!containsComponent(selectedComponent)) internalSelectComponent(first);
  ...
  public void selectNextComponent() {
    ...
    if (getComponents()[getComponentCount() - 1] == lastDisplayedComponent) return;
  ...
  public boolean selectComponent(Component selection) {
    if (!containsComponent(selection)) return false;
    ...
    // If we find the component we're selecting before hitting the visual gallery items, then we
    // need to travel
    // backward- otherwise, we need to travel forward
    for (Component component : getComponents()) {
      if (component == getSelectedComponent()) isVisible = true;

      if (component == selection) {
        if (!isVisible) isAfterCurrentSelection = false;
        break;
    ...
    if (isAfterCurrentSelection) return seekForwardToComponent(selection);
    else return seekBackwardToComponent(selection);
  }
  ...
  protected boolean seekForwardToComponent(Component component) {
    ...
    do {
      ...
      if (getSelectedComponent() != lastComponent) didMoveGallery = true;
