...
public class UIUtils {
   ...
   */
  public static void populateLanguageSelector(
      String defaultLocaleText,
      JComboBox<LanguageItem> languages,
      ResourceLoader resources,
      TechnicSettings settings) {
    // Add the system default locale as the first item
    languages.addItem(
        new LanguageItem(ResourceLoader.DEFAULT_LOCALE, defaultLocaleText, resources));
    ...
    for (Locale locale : supportedLanguages) {
      languages.addItem(
          new LanguageItem(
              resources.getCodeFromLocale(locale),
              locale.getDisplayName(locale),
              resources.getVariant(locale)));
    }
