...
public class CachedDiscordApi implements IDiscordApi {
  ...
  public void retrieveServer(
      ModpackModel modpack, final String serverId, final IDiscordCallback callback) {
    ...
    // Fill the dead cache immediately to prevent Discord API requests from getting spammed before
    // the first one
    // comes back. The response will sort out the "correct" value for the dead cache.
    deadCache.put(serverId, true);
    this.innerApi.retrieveServer(
        modpack,
        serverId,
        (pack, server) -> {
          if (server == null) {
