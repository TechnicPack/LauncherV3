...
public class ModpackZipFilter implements IZipFileFilter {
  ...
  public ModpackZipFilter(ModpackModel pack) {
    ...
    Stream.of("modpack.jar", "version.json", "runData")
        .forEach(filename -> allowedInBinDir.add(this.binPath.resolve(filename)));
  }
  ...
  private void warnAboutFile(File file) {
    Utils.getLogger()
        .log(
            ...
            "Modpack "
                + this.pack.getName()
                + " tried to extract reserved file "
  ...
  public boolean shouldExtract(String fileName) {
    ...
    // - The cache/ folder and anything inside it, which the launcher uses as storage for the
    // modpack mods
    File file = new File(this.pack.getInstalledDirectory(), fileName);

    // We use a Path here so we can properly compare paths instead of Java trying to compare them as
    // strings,
