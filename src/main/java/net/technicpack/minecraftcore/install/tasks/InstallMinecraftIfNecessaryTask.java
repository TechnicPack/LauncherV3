...
public class InstallMinecraftIfNecessaryTask extends ListenerTask<IMinecraftVersionInfo> {
  ...
  public InstallMinecraftIfNecessaryTask(
      ModpackModel pack, String minecraftVersion, Path cacheDirectory, boolean forceRegeneration) {
    this.pack = pack;
  ...
  public void runTask(InstallTasksQueue<IMinecraftVersionInfo> queue)
    ...
    if (dls == null) {
      throw new RuntimeException(
          "Using legacy Minecraft download! Version id = "
              + version.getId()
              + "; parent = "
  ...
  private static void copyMinecraftJar(Path jar, Path output, DownloadListener listener)
      throws IOException {
    String[] security = {"MOJANG_C.DSA", "MOJANG_C.SF", "CODESIGN.RSA", "CODESIGN.SF"};
    Pattern securityPattern =
        Pattern.compile(
            Arrays.stream(security).map(Pattern::quote).collect(Collectors.joining("|")));
