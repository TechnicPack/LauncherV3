...
public class CleanupAndExtractModpackTask implements IInstallTask<IMinecraftVersionInfo> {
  ...
  public CleanupAndExtractModpackTask(
      ModpackModel pack,
      Modpack modpack,
      ITasksQueue<IMinecraftVersionInfo> checkModQueue,
      ITasksQueue<IMinecraftVersionInfo> downloadModQueue,
      ITasksQueue<IMinecraftVersionInfo> copyModQueue) {
    this.pack = pack;
  ...
  public void runTask(InstallTasksQueue<IMinecraftVersionInfo> queue) throws IOException {
    ...
    // If we're installing a new version of modpack, then we need to get rid of the existing
    // version.json
    File versionFile = new File(binDir, "version.json");
    removeFile(versionFile);

    // Remove bin/install_profile.json, which is used by ForgeWrapper to install Forge in Minecraft
    // 1.13+
    ...
    // one that doesn't. One example of this is updating BareBonesPack from a Forge to a Fabric
    // build.
    ...
    for (Mod mod : modpack.getMods()) {
      ...
      if (md5 != null && !md5.isEmpty()) verifier = new MD5FileVerifier(md5);
      else verifier = new ValidZipFileVerifier();

      EnsureFileTask<IMinecraftVersionInfo> ensureFileTask =
          new EnsureFileTask<>(downloadModQueue, cacheFile)
              .withUrl(url)
