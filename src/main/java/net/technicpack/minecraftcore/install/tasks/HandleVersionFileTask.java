...
public class HandleVersionFileTask implements IInstallTask<IMinecraftVersionInfo> {
  ...
  public HandleVersionFileTask withCheckLibraryQueue(
      ITasksQueue<IMinecraftVersionInfo> checkLibraryQueue) {
    this.checkLibraryQueue = checkLibraryQueue;
    return this;
  }

  public HandleVersionFileTask withDownloadLibraryQueue(
      ITasksQueue<IMinecraftVersionInfo> downloadLibraryQueue) {
    this.downloadLibraryQueue = downloadLibraryQueue;
    return this;
  }

  public HandleVersionFileTask withCopyLibraryQueue(
      ITasksQueue<IMinecraftVersionInfo> copyLibraryQueue) {
    this.copyLibraryQueue = copyLibraryQueue;
    return this;
  }

  public HandleVersionFileTask withCheckNonMavenLibsQueue(
      ITasksQueue<IMinecraftVersionInfo> checkNonMavenLibsQueue) {
  ...
  public String getTaskDescription() {
    if (libraryName == null) return "Processing version.";
    else return "Verifying " + libraryName + ".";
  }
  ...
  public void runTask(InstallTasksQueue<IMinecraftVersionInfo> queue)
    ...
    // In Forge 1.13+ and 1.12.2 > 2847, there's an installer jar, and the universal jar can't be
    // used since it
    // doesn't have the required dependencies to actually launch the game.
    // So, for Forge 1.13+ we need to use ForgeWrapper to install Forge (it performs deobfuscation
    // at install time).
    // For Forge 1.12.2 it launches directly, as long as we inject the universal jar (it won't
    // launch at all if
    ...
    if (hasModernMinecraftForge || hasNeoForge) {
      ...
      if (is1_12_2) {
        for (Library library : version.getLibrariesForCurrentOS(launchOptions, javaRuntime)) {
          // For Minecraft Forge 1.12.2 > 2847, we correct the classifier for the library to
          // "universal".
          // The Forge installer just grabs this jar from within itself ("maven" folder), but we
          // grab it
          // directly from the Minecraft Forge Maven repo.
          // The Minecraft Forge universal jar is already specified in the version.json but doesn't
          // have the
          // "universal" classifier, so we change the name to add the "universal" classifier (for
          // download
      ...
      List<Library> dedupedInstallerLibraries =
          installerLibraries.stream()
              .filter(distinctByKey(Library::getName))
              .collect(Collectors.toList());

      for (Library library : dedupedInstallerLibraries) {
        if (library.isMinecraftForge() && is1_12_2) {
          // Handling of the modern 1.12.2 Minecraft Forge universal jar is done above, in the
          // version.json, rather than the install_profile.json
          continue;
        }

        // For Minecraft Forge 1.13+ up to 1.17, the URL for the universal jar isn't set, so we set
        // one here
        ...
        checkLibraryQueue.addTask(
            new InstallVersionLibTask(
                library,
                checkNonMavenLibsQueue,
                downloadLibraryQueue,
                copyLibraryQueue,
                pack,
                fileSystem));
      }

      // For Minecraft Forge 1.13+ and NeoForge, we inject our ForgeWrapper as a dependency and
      // launch it through that
      if (!is1_12_2) {
        ...
        for (Library library : version.getLibrariesForCurrentOS(launchOptions, javaRuntime)) {
          ...
          if (library.getGradleGroup().equals("net.minecraftforge")
            ...
            if (artifact.getUrl() == null || artifact.getUrl().isEmpty()) {
              // Modify the URL in-place, the altered library will get added by the rest of the
              // logic
              artifact.setUrl("https://maven.minecraftforge.net/" + library.getArtifactPath());
            }
          ...
          // Minecraft Forge 49.0.4+ sets a "client" library (I presume it's for the finalized MC
          // jar),
    ...
    for (Library library : version.getLibrariesForCurrentOS(launchOptions, javaRuntime)) {
      ...
      if (library.isLog4j()
        ...
        switch (log4jVersion) {
          ...
          default:
            throw new RuntimeException(
                "Unknown log4j version " + log4jVersion + ", cannot continue");
        }
        String url =
            String.format(
                TechnicConstants.TECHNIC_LIB_REPO
                    + "org/apache/logging/log4j/%1$s/%2$s/%1$s-%2$s.jar",
                artifactName,
                log4jVersion);
        Library fixedLog4j =
            new Library(
