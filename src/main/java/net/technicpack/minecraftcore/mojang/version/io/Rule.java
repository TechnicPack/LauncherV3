...
public class Rule {
  ...
  private static final String OS_VERSION_RANGE_FIELD = "versionRange";
  private static final String OS_VERSION_RANGE_MIN_FIELD = "min";
  private static final String OS_VERSION_RANGE_MAX_FIELD = "max";
  ...
  public static boolean isAllowable(
      List<Rule> rules, ILaunchOptions options, IJavaRuntime runtime) {
  ...
  public Rule(JsonObject rule) {
    ...
    if (rule.has(OS_FIELD)) {
      ...
      OsVersionRange versionRange = null;
      if (os.has(OS_VERSION_RANGE_FIELD)) {
        JsonObject versionRangeObject = os.get(OS_VERSION_RANGE_FIELD).getAsJsonObject();
        versionRange =
            new OsVersionRange(
                versionRangeObject.has(OS_VERSION_RANGE_MIN_FIELD)
                    ? versionRangeObject.get(OS_VERSION_RANGE_MIN_FIELD).getAsString()
                    : null,
                versionRangeObject.has(OS_VERSION_RANGE_MAX_FIELD)
                    ? versionRangeObject.get(OS_VERSION_RANGE_MAX_FIELD).getAsString()
                    : null);
      }
      conditions.add(
          new OsCondition(
      ...
      for (String feature : features.keySet()) {
        switch (feature) {
          case HAS_CUSTOM_RESOLUTION:
            conditions.add(
                new CustomResolutionCondition(features.get(HAS_CUSTOM_RESOLUTION).getAsBoolean()));
            break;
          case IS_DEMO_USER:
          ...
          case HAS_QUICK_PLAYS_SUPPORT:
            conditions.add(
                new QuickPlaysSupportCondition(
                    features.get(HAS_QUICK_PLAYS_SUPPORT).getAsBoolean()));
            break;
          case IS_QUICK_PLAY_SINGLEPLAYER:
            conditions.add(
                new QuickPlaySingleplayerCondition(
                    features.get(IS_QUICK_PLAY_SINGLEPLAYER).getAsBoolean()));
            break;
          case IS_QUICK_PLAY_MULTIPLAYER:
            conditions.add(
                new QuickPlayMultiplayerCondition(
                    features.get(IS_QUICK_PLAY_MULTIPLAYER).getAsBoolean()));
            break;
          case IS_QUICK_PLAY_REALMS:
            conditions.add(
                new QuickPlayRealmsCondition(features.get(IS_QUICK_PLAY_REALMS).getAsBoolean()));
            break;
          default:
  ...
  private static class OsCondition implements RuleCondition {
    ...
    private final OsVersionRange versionRange;
    private final String arch;

    OsCondition(String name, String version, OsVersionRange versionRange, String arch) {
      this.name = name;
    ...
    @Override
    public int hashCode() {
      return Objects.hash(name, version, versionRange, arch);
    }
    ...
    public void serialize(JsonObject root) {
      ...
      if (versionRange != null) os.add(OS_VERSION_RANGE_FIELD, versionRange.serialize());
      if (arch != null) os.addProperty(OS_ARCH_FIELD, arch);
      root.add(OS_FIELD, os);
    }
  }

  private static class OsVersionRange {

    private final String min;
    private final String max;

    OsVersionRange(String min, String max) {
      this.min = min;
      this.max = max;
    }
    ...
    public boolean equals(Object o) {
      if (this == o) return true;
      if (o == null || getClass() != o.getClass()) return false;
      OsVersionRange that = (OsVersionRange) o;
    ...
    JsonElement serialize() {
      JsonObject json = new JsonObject();
      if (min != null) json.addProperty(OS_VERSION_RANGE_MIN_FIELD, min);
      if (max != null) json.addProperty(OS_VERSION_RANGE_MAX_FIELD, max);
      return json;
    }
    ...
    private static int compareDottedVersion(String left, String right) {
      ...
      for (int i = 0; i < length; i++) {
        long leftPart = parseVersionPart(leftParts, i);
        long rightPart = parseVersionPart(rightParts, i);
