...
public class RunData {
  ...
  public boolean isJavaValid(String testString) {
    ...
    if (compareString == null || compareString.isEmpty()) compareString = "1.6";
  ...
  public IJavaRuntime getValidJavaVersion(JavaVersionRepository repository) {
    ...
    if (best64Bit != null && isJavaValid(best64Bit.getVersion())) return best64Bit;
    else if (best64Bit == null && requires64Bit) return null;
  ...
  private Memory getMemorySetting(long memory) {
    ...
    Memory maxMemoryOption =
        Arrays.stream(Memory.memoryOptions)
            .max(Comparator.comparingLong(Memory::getMemoryMB))
            .orElseThrow(() -> new RuntimeException("No memory options available"));
    Utils.getLogger()
        .log(
            Level.WARNING,
            "Invalid runData memory value specified ("
                + memory
                + " MB); using maximum possible instead ("
                + maxMemoryOption.getMemoryMB()
                + " MB)");
