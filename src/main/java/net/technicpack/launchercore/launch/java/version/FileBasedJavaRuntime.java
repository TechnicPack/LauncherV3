...
public final class FileBasedJavaRuntime implements IJavaRuntime {
  ...
  private void getInformationFromJavaRuntime() {
    ...
    if (javaBinaryPath.endsWith("javaw.exe")) {
      // On Windows operating systems, we ask people to find javaw.exe, which we use to actually run
      // anything.
      // However, javaw.exe doesn't output anything since it's designed to be quiet, only java.exe
      // does, so
