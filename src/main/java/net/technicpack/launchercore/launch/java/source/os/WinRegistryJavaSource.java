...
public class WinRegistryJavaSource implements IVersionSource {
  ...
  public void enumerateVersions(JavaVersionRepository repository) {
    ...
    // Azul Zulu
    enumerateRegistryForBitness(
        repository, "32", "Software\\Azul Systems\\Zulu", "InstallationPath");
    enumerateRegistryForBitness(
        repository, "64", "Software\\Azul Systems\\Zulu", "InstallationPath");

    // BellSoft Liberica
    enumerateRegistryForBitness(
        repository, "32", "Software\\BellSoft\\Liberica", "InstallationPath");
    enumerateRegistryForBitness(
        repository, "64", "Software\\BellSoft\\Liberica", "InstallationPath");
  }

  private void enumerateRegistryForBitness(
      JavaVersionRepository repository, String bitness, String keyPath, String keyName) {
    String output =
    ...
    for (String line : output.split("\\r?\\n")) {
      ...
      // The type is always REG_SZ because that's what we searched for, so we find the type & expect
      // the value to be afterward
      ...
      repository.addVersion(
          new FileBasedJavaRuntime(
              new File(path + File.separator + "bin" + File.separator + "javaw.exe")));
    }
  }
}
