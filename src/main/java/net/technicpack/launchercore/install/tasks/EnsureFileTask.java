...
public class EnsureFileTask<T> implements IInstallTask<T> {
  ...
  public EnsureFileTask(@NotNull ITasksQueue<T> downloadQueue, @NotNull File target) {
    this.downloadTaskQueue =
        Objects.requireNonNull(downloadQueue, "Download task queue must be set.");
  ...
  public void runTask(InstallTasksQueue<T> queue) throws IOException {
    ...
    if (zipExtractionDirectory != null) {
      // This is added before the download because we're adding to the head, so the download task
      // runs before
    ...
    if (sourceUrl != null
        && (!targetFile.exists()
            || (fileVerifier != null && !fileVerifier.isFileValid(targetFile)))) {
      DownloadFileTask<T> downloadFileTask =
          new DownloadFileTask<>(sourceUrl, targetFile, fileVerifier, friendlyFileName);
   ...
   */
  public @NotNull EnsureFileTask<T> withExtractTo(
      @NotNull File directory, @NotNull ITasksQueue<T> copyQueue) {
