...
public class PlanExecutor<TContext> {
    ...
    public PlanExecutor(ExecutionProgressListener listener) {
        this(listener, Comparator.comparingInt((PlanNode<TContext> node) -> node.getPhase().getOrder())
                                 .thenComparing(PlanNode::getId));
    }

    public PlanExecutor(ExecutionProgressListener listener, Comparator<PlanNode<TContext>> readyNodeComparator) {
        this.listener = listener;
        this.readyNodeComparator = readyNodeComparator;
    }

    public void execute(ExecutionPlan<TContext> plan, TContext context) throws IOException, InterruptedException {
        ...
        while (!ready.isEmpty()) {
            ...
            for (String dependentId : dependents.getOrDefault(node.getId(), Collections.<String>emptyList())) {
    ...
    private class NodeState implements NodeProgressReporter {
        ...
        private void complete(float completedWeight, boolean isLastNode) {
            nodeProgress = 100.0f;
            if (listener != null) {
                listener.overallChanged(node.getPhase().getLabel(), totalWeight == 0.0f ? 100.0f : (completedWeight / totalWeight) * 100.0f);
